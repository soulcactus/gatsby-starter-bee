...
SVELTE에서 요소에 이벤트를 바인딩할 때는 'on:이벤트명' 패턴으로 작성해야 합니다.

이제 input 값이 변경될 때마다 todoValue 값이 업데이트됩니다. 상태 값이 잘 업데이트되고 있는지 확인하기 위해 SVELTE Chrome Extension을 설치합니다.
...
자세한 사항은 [이곳](https://svelte.dev/docs#each)을 참고하시기 바랍니다.
...
SVELTE에서 조건물은 작성할 때는 {#if ...}를 사용합니다. 패턴은 아래와 같습니다.
...
자세한 사항은 [이곳](https://svelte.dev/docs#if)을 참고하시기 바랍니다.
